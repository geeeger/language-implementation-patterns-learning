...
export default abstract class Parser {
    ...
    public alreadyParsedRule(memoization: Map<number, number>): boolean {
        const memoI: number | undefined = memoization.get(this.index());
        if (memoI === undefined) {
            return false;
        }
        const memo = memoI as number;
        console.log(`parsed list before at index ${this.index()};  skip ahead to token index ${memo}: ${this.lookahend[memo].text}`);
        if (memo === Parser.FAILED) {
            throw new Error(`PreviousParseFailedException`);
        }
        this.seek(memo);
        return true;
    }

    public memoize(memoization: Map<number, number>, startTokenIndex: number, failed: boolean) {
